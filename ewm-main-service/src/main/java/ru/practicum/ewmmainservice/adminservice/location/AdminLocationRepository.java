...
public interface AdminLocationRepository extends JpaRepository<Location, Long> {
    /*@Query("SELECT location FROM Location location WHERE location.lat BETWEEN :lat - :latDelta AND :lat + :latDelta " +
            "AND location.lon BETWEEN :lon - :lonDelta AND :lon + :lonDelta")*/
